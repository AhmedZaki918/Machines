...
import com.example.machines.databinding.FragmentCementMillThreeBinding
...
@AndroidEntryPoint
class CementMillThreeFragment : Fragment(), OnItemClick {

    private lateinit var binding: FragmentCementMillThreeBinding
    private lateinit var viewModel: CementThreeViewModel
    ...
    override fun onCreateView(
        ...
        binding = FragmentCementMillThreeBinding.inflate(inflater, container, false)

        binding.header.drawScreenHeader(getString(R.string.cement_mill_3), this)
        viewModel = ViewModelProvider(this)[CementThreeViewModel::class.java]
        userPreferences = UserPreferences(requireContext())

        viewModel.getAllCementThreeItems().observe(viewLifecycleOwner) { items ->
    ...
    override fun <T> onClicked(model: T) {
        model as CementMillMachine3
        // Save item in Constants Object class
        model.apply {
            cementMill_3 = CementMillMachine3(id, startTime, stopTime, reason, rh)
        }
        // Save machine name to determine which one to update before navigate
        machineType = Type.CEMENT_MILL_THREE.value
        findNavController()
            .navigate(R.id.action_cementMillThreeFragment_to_updateFragment)
    }

    override fun <T> onDeleted(model: T) {
        viewModel.deleteCementThree(model as CementMillMachine3)
    }


    private fun updateUi(items: List<CementMillMachine3>) {
        binding.rvCementMill3.adapter = CementMillThreeAdapter(items, this, false)
        rhTotal = updateRhTotal(items, binding.header)
        updateRunningStatusForMachine(items)

        // Save total rh for Cement Mill Three
        val rh = rhTotal!!.hours + COLUMN + formatTime(rhTotal!!.minutes)
        userPreferences.saveData(RH_CEMENT_MILL_3_KEY, rh)
    }


    private fun updateRhTotal(
        items: List<CementMillMachine3>,
    ...
    private fun switchVisibility(items: List<CementMillMachine3>) {
        binding.apply {
            if (items.isEmpty()) {
                updateRunningStatusForMachine(items)
                rvCementMill3.visibility = INVISIBLE
                ...
                rvCementMill3.visibility = VISIBLE
    ...
    private fun setClickListeners() {
        ...
        machineType = Type.CEMENT_MILL_THREE.value
        binding.fab.click {
            findNavController().navigate(R.id.action_cementMillThreeFragment_to_addFragment)
        }
    }


    private fun updateRunningStatusForMachine(items: List<CementMillMachine3>) {
        if (items.isEmpty()) {
            userPreferences.saveData(
                CEMENT_MILL_3_STATUS_KEY,
